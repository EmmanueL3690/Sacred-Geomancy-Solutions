...
export default function AdminSacredSolutions({ user, onLogout }) {
  ...
  const solutionsData = {
    ...
    "12A": "Ota po, yoo ri iwosi, sora fun ibinu papa lori owo. ONIBEERE ORE DIEDIE NI WOLE FUN",
    "13A": "Nkan ibinu, ti okan ko, ki ise ma dojuru, Alaniyan ngbero moto nla jeep lati ra",
    "14A": "(ALFA, ALADUA, TI MA NGBAWE, ONI MIMO,) Aya onibere nja aya lori ogun to fe se, yio je eniti Olorun fun lebun, onimimo ni, emi ti onse ogun.",
    "15A": "ONI GBESE LORUN TI NPALAYA, Oni gbese lorun ti o npa laya, ma se lo aso to riri, aso alakowe",
    "16A": "Inu bibi abiju, Igbona okan, Oro ti a nba o leru, yio dara, Oro ara eni sisu, OKAN ENI KUN FUN ERO ORISIRISI okan",
    "17A": "1@5 and,6 Oyun ibeji/oyun ti ko ni se see se",
    "18A": "1&2 1,1: Eti yio ba adawole re yio maa gbero nkan Kan ko ni seese. Yio ri wahala die, ko be Olorun Yio maa banujje",
  ...
  useEffect(() => {
    const unsubDetails = onSnapshot(collection(db, "userDetails"), snapshot => {
      setAllDetails(snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() })));
    });
    const unsubTestimonies = onSnapshot(collection(db, "userTestimonies"), snapshot => {
  ...
  const deleteDetail = async id => {
    if (confirm("Delete this user detail?")) await deleteDoc(doc(db, "userDetails", id));
  };

  const deleteTestimony = async id => {
    if (confirm("Delete this testimony?")) await deleteDoc(doc(db, "userTestimonies", id));
  };
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-grow p-4 sm:p-6 overflow-auto space-y-8">
        ...
        <div>
          ...
          <div className="space-y-4">
            {allDetails.map(detail => (
              <Card key={detail.id} className="p-4 flex justify-between items-center">
                <div>
                  <p className="font-bold">{detail.name}</p>
                  <p>Age: {detail.age}</p>
                  <p>Location: {detail.location}</p>
                  <p>Number Picked: {detail.numberPicked}</p>
                </div>
                <button
        ...
        {/* User Testimonies */}
        <div>
          <h2 className="text-xl font-bold mb-4">User Testimonies</h2>
          <div className="space-y-4">
